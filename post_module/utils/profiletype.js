...
module.exports.profiletype = async (userId,profileId) => {
    var data = {};
    var response={};
    var user1={};
    var bool=false;
    response = await User.find({ "_id": profileId })
        ...
        .catch((err) => {
            bool=true;
            data = { message: err.message, status: 500, output: false,allowed:false }
            return data;
        })
    ...
    if(!bool){
        data = { message: "This account is private", status: 200, output: true ,allowed:false}
        return data;
    }
